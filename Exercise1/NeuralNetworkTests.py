...
class TestFullyConnected1(unittest.TestCase):
    ...
    def test_forward_size(self):
        layer = FullyConnected.FullyConnected(self.input_size, self.output_size)
        output_tensor = layer.forward(self.input_tensor)  # size - batch_size*output_size
...
class TestSoftMax(unittest.TestCase):
    ...
    def setUp(self):
        self.batch_size = 9
        self.categories = 4
        self.label_tensor = np.zeros([self.batch_size, self.categories])  # size [9,4]
        for i in range(self.batch_size):
    ...
    def test_forward_shift(self):
        input_tensor = np.zeros([self.batch_size, self.categories]) + 10000.  # size [9,4]
        layer = SoftMax.SoftMax()
